...
import random
def counter(img_path,threshold=200):
...
def main():
    ...
    for _ in os.listdir(args.data_dir):
        ...
        if(name!="" and name!=data_parameter.name):
            print(f"Img {data_parameter.name} and Count {count}")
            print(f"MAE: {np.mean(ae)}")
            print(f"MSE: {np.sqrt(np.mean(se))}")
            count=count+1
            if(count==70):
              break
            # mae=mae+abs(diff)
            # mse=mse+(diff)**2
            # print(f"MAE is {mae/count} and MSE is {np.sqrt(mse/count)}")
            name=data_parameter.name
            # diff=0
    # count=count+1
    # mae=mae+abs(diff)
